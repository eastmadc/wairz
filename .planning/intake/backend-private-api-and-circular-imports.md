...
**Partial P3 progress — session aa8b4a17 (2026-04-24, commits `8f9d261`, `4bd491b`, `9a26c1a`):** 7th/8th/9th P3 carve-outs shipped as a single-session triple-play — user-directed continuation past the Rule #19 stop-point the session-78f772bd extraction recommended ("burn down risk first, deep research at each step and across steps"). Risk-ordered execution (lowest blast-radius first): `clamav_service.py` → `attack_surface_service.py` → `hardware_firmware/cve_matcher.py`.

- `clamav_service.py` (2 → 0): two duplicate `from app.config import get_settings` function-body imports (check_available L31 + _get_clamd L49) collapsed to a single top-level import. `import clamd` intentionally retained as function-local per Rule #30 caveat (a) — optional third-party dep; bare top-level import would break environments without python-clamd. app.config is the canonical @lru_cache singleton; no reverse-cycle to clamav_service. Transitive smoke confirmed top-level consumer `security_audit/hash_lookups` still imports cleanly.
- `attack_surface_service.py` (2 → 0): L140 `binary_analysis_service._LIEF_ELF_ARCH_MAP` + L193 `analysis_service.check_binary_protections` promoted. Runtime try/except around `check_binary_protections(path)` CALL preserved to keep malformed-ELF fail-soft semantics (only the import moved; the except-clause still catches pyelftools-raised exceptions). Orthogonal latent-bug noted in commit but NOT fixed per CLAUDE.md "don't expand scope": `_LIEF_ELF_ARCH_MAP` is a module-level empty dict populated only by `binary_analysis_service._ensure_lief()`, which attack_surface never invokes — arch resolution therefore depends on side-effect from other callers. Dict-by-reference semantics mean promotion is behavior-preserving regardless.
- `hardware_firmware/cve_matcher.py` (2 → 0): L251 `cpe_dictionary_service.CpeDictionaryService` (inside Tier 1 try/except that wraps import+instantiate+await) + L481 `hardware_firmware.kernel_vulns_index as kvi` (outside try/except; try wraps only the subsequent `await kvi.is_populated()`). `as kvi` alias preserved to keep all Tier 5 call sites unchanged (kvi.is_populated, kvi.lookup). Intra-subpackage cycle-check: `hardware_firmware/__init__.py` eager-imports only `detector`, which transitively reaches classifier + parsers + firmware_paths + models — none reach cve_matcher. Tier 1 network-bound fail-soft semantics preserved (try/except still wraps the ensure_loaded await).

Rule #31 width-canary: all three files pre-edit showed narrow == wide == wider = 2 — no under-counting detected (the recent firmware_service narrow=2 vs widened=14 incident did NOT repeat; running the width canary is cheap insurance regardless). Rule #30 reverse check: ZERO tests use `patch("app.services.{attack_surface_service,clamav_service,hardware_firmware.cve_matcher}.*")`. Rule #30 source-audit: all six promotion targets are pure-leaf or near-leaf — app.config is the @lru_cache singleton (only `clamav_host`/`port`/`enabled` fields referenced); binary_analysis_service top-level is stdlib-only (LIEF itself lazy-loaded inside _ensure_lief); analysis_service top-level is `elftools.elf.elffile` only (pyelftools is a hard dep present in all Docker environments); cpe_dictionary_service top-level adds `httpx`+`redis.asyncio`+app.config (no module-level network I/O; CpeDictionaryService class `__init__` is pure); kernel_vulns_index top-level adds `redis.asyncio`+`packaging.version`+app.config (same no-module-level-I/O property). Rule #11 import smokes green on backend + worker + downstream blast-radius (hash_lookups transitive for clamav; router + tool for attack_surface; router + tool + `backfill_detection.py` script for cve_matcher); identity checks (`original is promoted`) confirmed CpeDictionaryService + check_binary_protections + kvi all bind to their exact source objects with zero accidental rebinding. Rule #25 single-commit-per-file: 3 refactor commits + this log entry.

**Post-session repo-wide residual across `backend/app/services/` (ast.walk unique-line count): 12 across 12 files** (down from 18 across 15 files). Residual map is now a FLAT 1-per-file distribution: `abusech_service`, `binary_analysis_service`, `comparison_service`, `device_service`, `hardware_firmware/classifier`, `hardware_firmware/graph`, `qiling_service`, `sbom/enrichment`, `sbom/strategies/syft_strategy`, `security_audit/network`, `virustotal_service`, `yara_service`.

**Rule #19 stop-point reached (stronger than the session-78f772bd recommendation):** 9 P3 carve-outs across 3 calendar sessions; residual distribution is now structurally flat at 1-per-file. Each additional carve-out would net -1 on the repo-wide count at the cost of a full per-call Rule #30 audit + Rule #11 smoke + separate commit. Classic diminishing returns; the intake's acceptance criteria ("audit-based, scoped per service pair, not bulk") is structurally satisfied at this boundary. **Recommendation: CLOSE the P3 thread** unless a specific cycle-pressure event surfaces — a failed CI import, a new circular-import ImportError at module load, or a cold-start performance regression attributable to one of the 12 residual sites. Aesthetic tidiness is not pressure (Rule #19).
