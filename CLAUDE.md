...
44. **Every new walker MCP category MUST ship ONE `lookup_<artefact>_across_firmwares` tool alongside the per-firmware tools — the cross-firmware aggregation belongs at walker-stream time, not as a follow-up.** Wairz's unique competitive wedge against EZTools / Plaso / Velociraptor / dissect-CLI is **MCP-callable cross-firmware aggregation** — operators can ask "which firmwares contain the same kernel-driver hash" or "which firmwares share a known-malicious WMI consumer payload" in one MCP call against the firmware corpus. The aggregation pattern is mechanical: SQL `JOIN firmware ON Artefact.firmware_id = firmware.id` + group-by-firmware bucket with `match_count` + optional `supply_chain_signal` flag when match_count >= 2. **Mechanical shape (mirror exactly):** function name `_handle_lookup_<artefact>_across_firmwares(input, context)`; registers as `lookup_<artefact>_across_firmwares` in the MCP tool category; input schema accepts a `query` filter + optional `scope` ∈ {`project`, `global`}; output is one row per matching firmware with `firmware_id`, `project_id`, `match_count`, `sample_<artefact>`, and a top-level `supply_chain_signal: bool`. Reference precedent: `lookup_systemd_unit_across_firmwares` at `backend/app/ai/tools/linux_systemd.py:437,772` (ι.B.E original codification). **Rule-of-Nine DURABLE BEYOND DEBATE evidence:** ι.B.E systemd (Rule-of-One) + ι.C.E ETL (Two) + ι.D.E EFS (Three) + ι.E.E container (Four, DURABLE) + κ.A journald backfill (Five, BACKFILL precedent — Rule applies retroactively to walkers shipped before its codification) + κ.B.E AppCompat (Six) + κ.C.E linux_persistence (Seven) + κ.D.E DPAPI (Eight) + κ.E.E UsnJrnl (Nine). **11 Rule-#44 backfill candidates remain** across η/θ/pre-ι walkers per κ Scout 2's audit (windows_event_log, windows_mft, windows_lnk, windows_scheduled_task, windows_prefetch, windows_srum, windows_registry, plus θ.A/B/C/E walkers that may already-fit under different naming) — each is a single-commit MCP tool addition, queued for a κ.X or λ-idle-slot maintenance batch. **Mechanical detection at walker-stream time:** if `register_<category>_tools(registry)` doesn't include `lookup_*_across_firmwares`, the .E commit is incomplete. **Companions:** Rule #16 detection roots (cross-firmware iterates over multiple firmware rows from the same project), Pattern P1 single-sub-agent + precedent reuse (the cross-firmware tool inherits shape from prior streams), Pattern P5 per-piece direct-push (the cross-firmware tool is part of the .E commit, not a separate stream). Promotion date: 2026-05-12 (κ campaign close — windows-coverage-godmode-kappa-2026-05-12).

45. **Security-sensitive metadata walkers MUST be PARSE-ONLY — surface METADATA without decryption, execution, or plaintext-recovery — enforced by a Rule #36 EXTENSION test gate scanning walker source for forbidden tokens via `tokenize`.** When a walker targets an artefact whose contents are security-sensitive (DPAPI master keys, EFS DDF/DRF, BitLocker recovery info, PGP keyring metadata, TPM blobs, certificate trust stores, OpenSSL key bundles), the walker SHALL surface METADATA ONLY: identifiers (GUIDs, fingerprints), flags, sizes, ownership SIDs, timestamps, count-of-N — NEVER decrypted payload, NEVER attempted decryption, NEVER plaintext recovery. The walker is the SECURITY BOUNDARY; running decrypt code inside the wairz worker container would: (a) require the decryption key (which is the credential the artefact *is* — circular); (b) make the wairz worker a credential-stealing target; (c) violate the parse-only model that operator-trusted forensic tools (Velociraptor, dissect.*) follow by default. **Required test gate `test_walker_no_decrypt` (Rule #36 EXTENSION):** tokenize the walker source via Python's `tokenize` module (strip comment + docstring tokens — they may legitimately *mention* decrypt for documentation), then assert ZERO matches against a forbidden-token regex covering all known decrypt entry points (e.g. `\.decrypt\s*\(`, `CryptUnprotectData`, `cryptography.fernet`, `Crypto.Cipher`, `impacket\.dpapi\.MasterKey\(`, `pyDes`, library-specific decrypt class names per artefact family). **CRITICAL — gate-canary requirement (Rule #46 partner):** the test gate's regex MUST be whitespace-tolerant (`\.\s*decrypt\s*\(` not `\.decrypt\(`) because `tokenize` joins tokens with single spaces (`obj.decrypt(` becomes `obj . decrypt (` after token-string-join). The walker test file MUST also include a companion canary test `test_walker_no_decrypt_gate_actually_fires` that synthesizes a forbidden-token violation IN MEMORY (concatenated code lines — NOT an f-string, since `tokenize` strips the string-literal entirely) and confirms the gate WOULD catch it. Without the canary, the gate is a Rule #17 silent-pass instance. **Rule-of-Two DURABLE BEYOND DEBATE evidence:** ι.D EFS DDF/DRF walker (Rule-of-One, 2026-05-12 — `backend/app/services/efs_walker.py` + `test_efs_walker.py::test_walker_no_decrypt`); κ.D DPAPI master-key walker (Rule-of-Two, 2026-05-12 — `backend/app/services/dpapi_walker.py` + `test_dpapi_walker.py::test_walker_no_decrypt` with whitespace-tolerant regex + canary that confirmed ι.D's regex would have ALSO failed under the same tokenize-whitespace condition → ι.D backfill action item filed). **Future application** (Rule #45 + Rule #46 partner): any security-sensitive metadata walker shipped from κ onwards inherits this discipline. Future candidates: BitLocker recovery key metadata, PGP keyring metadata, TPM PCR / sealed-blob metadata, OpenSSL private-key file metadata, SSH host-key fingerprints. **Companions:** Rule #36 no-execute base (security boundary discipline that Rule #45 extends to no-decrypt), Rule #46 canary discipline (Rule #45 requires Rule #46 for the gate to be trustworthy), Rule #19 evidence-first (probe the artefact format with `xxd` / `inspect.getsource` BEFORE writing the parser — avoid assumed-format bugs). Promotion date: 2026-05-12 (κ.D close).

46. **Any verification mechanism that asserts ABSENCE of something (no forbidden tokens, no type errors, no failed tests, no policy violations) MUST have a CANARY test that introduces a synthetic violation and confirms the mechanism actually catches it.** Without the canary, a passing result is structurally indistinguishable from "the mechanism isn't actually checking." This generalises Rule #17 (silent-CLI-exit canary for `tsc -b` cache short-circuit) + Rule #24 (mandatory tsc `-b --force` canary for `--noEmit` empty-`files` no-check) to ALL verification mechanisms: ruff/lint suppressions, Rule #36/#45-style test gates, CI workflows, pre-commit hooks, type-checker invocations, deny-list scanners, AST/token visitors, schema-validation assertions. **Mechanical shape:** for every gate that says "asserts X is absent", ship a paired canary test in the same test file that (a) synthesizes a violation IN MEMORY or as a fixture, (b) runs the gate against the synthetic input, (c) asserts the gate would REJECT it. The canary lives in the same file as the gate it protects — never a separate "general canary suite" (loses proximity + freshness). **Rule-of-Four evidence chain:** (i) Rule #17 (original 2026-04-22) — `tsc -b` incremental cache exits 0 on no-change; canary feeds a known-bad TS file (`const x: number = "str"`) and confirms tool exits non-zero. (ii) Rule #24 (2026-04-23) — `npx tsc --noEmit` silently no-checks the `frontend/tsconfig.json` (which has `"files": []` + `"references"`); mandatory once-per-session canary writes `frontend/src/__canary.ts` with a deliberate type error and confirms `npx tsc -b --force` fails. (iii) κ.D test-gate canary (2026-05-12) — Rule #36 EXTENSION (no-decrypt) gate's naive regex `\.decrypt\(` failed against the synthetic violation because `tokenize` joins tokens with single spaces (`obj.decrypt(` → `obj . decrypt (`); canary CAUGHT the latent bug + drove the whitespace-tolerant fix; **retroactively exposed that ι.D EFS gate had the same weakness → backfill action filed.** (iv) κ.E meta-canary (2026-05-12) — Rule #24 canary execution itself triggered a Rule #35a pipe-induced silent exit (`tsc 2>&1 | tail -10` hid the real tsc exit code); the canary's failure mode self-surfaced and was fixed by re-running without the pipe. **Mechanical detection during a new gate's authoring:** before writing the gate's assertion, write the SYNTHETIC-VIOLATION CONSTRUCTOR first; ensure the synthetic actually round-trips through whatever serialization the gate uses (string-join, tokenize, AST-walk, JSON-load, etc.) — a synthetic that the gate's preprocessing strips out (e.g. an f-string violation under tokenize) is itself the bug, and finding it pre-ships saves regressions. **Companions:** Rule #17 (the original instance — Rule #46 is its generalisation), Rule #24 (the mandatory-per-session form for tsc), Rule #36 (the no-execute base that Rule #45 + Rule #46 jointly protect — gates without canaries are silent passes), Rule #45 (the canary requirement applies wherever a security-sensitive walker has a no-decrypt gate). **NEW Rule-of-Four candidate watching for Rule-of-Five extension:** ANY future verification gate written without a paired canary is a Rule #46 violation; reviewer-level check at PR/commit time is "where's the canary that proves this gate fires on a synthetic?" Promotion date: 2026-05-12 (κ.D + κ.E close).
