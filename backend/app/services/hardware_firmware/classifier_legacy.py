"""Legacy classifier — revert-safety shim for the P3.1.h cut-over.

This module is a frozen copy of the pre-P3.1.h ``classifier.py`` body.
Kept available for ~1 release as a recovery surface if the catalog
cut-over needs to be rolled back without a git revert. Callers can swap
``from app.services.hardware_firmware import classifier`` for
``from app.services.hardware_firmware import classifier_legacy as
classifier`` to reach this code.

DO NOT add new functionality here. New classifier behaviour belongs in
