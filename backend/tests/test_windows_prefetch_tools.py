...
def test_register_windows_prefetch_tools_includes_cross_firmware_lookup():
    """Rule #44 acceptance — register_windows_prefetch_tools registers
    lookup_prefetch_record_across_firmwares alongside the per-firmware
    tools. Issue #15 backfill for ζ.2 walker."""
    from app.ai.tool_registry import ToolRegistry

    reg = ToolRegistry()
    register_windows_prefetch_tools(reg)
    names = {tool.name for tool in reg._tools.values()}
    assert "lookup_prefetch_record_across_firmwares" in names
