...
- `.mex/context/{architecture,stack,conventions,decisions,mcp-tools}.md` — dense, YAML-fronted context files with `triggers` + `edges` for graph navigation. The Verify Checklist in `conventions.md` is the task-time gate derived from rules 1–28 above.
