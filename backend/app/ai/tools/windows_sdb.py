...
async def _handle_lookup_sdb_entry_across_firmwares(
    input: dict, context: ToolContext
) -> str:
    """**CROSS-FIRMWARE AGGREGATION** — given a fingerprint_sha256
    (or file_sha256), return ALL firmware images in the active project
    (or globally) with a matching SDB entry.

    Rule #44 — the fingerprint_sha256 column is specifically designed
    for cross-firmware correlation (per the θ.D.A model docstring:
    "campaign / supply-chain correlation across the wairz corpus").
    Same fingerprint across firmware ⇒ same shim was planted (T1546.011
    Application Shimming campaign indicator).

    Companion to ``lookup_sdb_shim`` — that tool surfaces a flat
    cross-corpus row list keyed by (firmware_id, file_path, shim_class);
    THIS tool follows the Rule #44 canonical shape (one row per
    matching firmware with match_count + sample_entry +
    ...
    - match_count (matching SDB rows in that firmware)
    ...
    if fingerprint_sha256:
        where.append(WindowsSdbEntry.fingerprint_sha256 == fingerprint_sha256)
    if file_sha256:
    ...
    if scope == "project":
        ...
        stmt = (
            select(WindowsSdbEntry, Firmware, Project)
            .join(Firmware, WindowsSdbEntry.firmware_id == Firmware.id)
    ...
    for rec, firmware, project in rows:
        ...
        if slot["sample_entry"] is None:
            slot["sample_entry"] = {
                ...
                "sdb_kind": rec.sdb_kind,
                "app_name": rec.app_name,
                "app_exe": rec.app_exe,
                "shim_class": rec.shim_class,
    ...
    out: dict = {
        ...
        "data_only_disclaimer": (
            ...
            "parsed shims via sdbinst / AppHelp / Mscoree."
        ),
    }
    if not matches:
        out["message"] = (
            "No matching SDB entries found. Ensure the walker has run "
            "on the relevant firmwares (trigger_sdb_walk + poll). The "
            "fingerprint_sha256 is the θ.D.A walker's stable "
            "cross-firmware identity key (T1546.011 Application "
            "Shimming campaign correlation across the wairz corpus)."
        )
...
def register_windows_sdb_tools(registry: ToolRegistry) -> None:
    ...
    registry.register(
        name="lookup_sdb_entry_across_firmwares",
        description=(
            "Rule #44 CROSS-FIRMWARE AGGREGATION (canonical-shape "
            "companion to lookup_sdb_shim) — given a fingerprint_sha256 "
            "(or file_sha256), return ALL firmware images in the active "
            "project (or globally, with scope='global') that contain a "
            "matching SDB entry. Identity key is fingerprint_sha256, "
            "the θ.D.A walker's stable cross-corpus correlation column. "
            "Returns one entry per matching firmware with match_count, "
            "sample_entry, any_anomaly flag, and "
            "supply_chain_signal=True when match_count >= 2 firmwares "
            "(T1546.011 Application Shimming campaign indicator). Use "
            "lookup_sdb_shim for the alternative flat row-list shape "
            "with shim_class filtering."
        ),
        input_schema={
            "type": "object",
            "properties": {
                "fingerprint_sha256": {
                    ...
                    "description": (
                        ...
                        "tuple — the θ.D.A walker's cross-corpus key."
                    ),
                },
                "file_sha256": {
                    "type": "string",
                    "description": (
                        ...
                        "SHA256 hex string of the raw .sdb file content."
                    ),
                },
                "scope": {
                    "type": "string",
