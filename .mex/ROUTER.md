...
- `cve-match-residual-oom-2026-04-25.md` — HIGH priority, observation-only intake from session 56797be2; describes the post-Bug #1 remaining OOM in `/cve-match`, ranks Tier 4 / Tier 5 hypotheses, suggests instrumentation-first diagnostic step before any refactor.

**Known issues:**
- `/cve-match` endpoint un-shippable from GUI: returns HTTP=000 / kills backend container at ~85 s on fw a7523429 (RespArray V1.12, 11 chipset-tagged blobs). Bug #1's singleton fix is in place and verified (eliminates 11× CPE dictionary loads), but the residual cause is unknown. Workaround: do not invoke `/cve-match` from the GUI on multi-blob firmware until `cve-match-residual-oom-2026-04-25.md` is resolved.
