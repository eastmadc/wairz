...
| M-3 | ✅ shipped 2026-05-06 | Highest-risk site `SbomPage.tsx` `VirtualizedVulnTable` shipped 2026-05-05 (commit `baadedb`). Residual sites — width-canary corrected the count from "7" to 6 (Rule #31 instance — intake overcounted by 1) — migrated 2026-05-06: `BlobTable.tsx`, `DriversTable.tsx`, `CvesTab.tsx`, `ComparisonPage.tsx`, `SecurityScanResults.tsx`, `SecurityScanPage.tsx`. Pattern extracted to a reusable hook `frontend/src/hooks/useResponsiveListHeight.ts` (commit `88f3ed0`); 6 consumer sites migrated in commit `ab86342`. Each consumer wraps its `<List>` in a `<div ref={ref}>` and passes `style={{ height, minHeight: <floor> }}` — the hook measures `window.innerHeight - rect.top - 32` clamped to `[minHeight, maxHeight]` and re-measures on resize + parent ResizeObserver. Width-canary post-migration: `calc\(100vh\s*-\s*[0-9]+px\)` narrow grep returns 1 hit (an explanatory comment in `SbomPage.tsx:1163`, not live code), down from 6 pre-migration. The remaining 7 `h-[calc(100vh-Xrem)]` Tailwind class shells (HardwareFirmwarePage, FindingsPage, ComponentMapPage, ExplorePage, SecurityToolsPage) are page-level flex layouts, not virtualized lists — out of M-3 scope. Rule #24 mandatory tsc canary fired (exit=2 on planted error) before trusting any "0 errors" output; per-file typecheck after each edit (Rule #22). Rule #26 frontend rebuild done; container healthy; new bundle hash `index-DBw3f-j1.js` served; hook compiled into its own chunk `useResponsiveListHeight-ggioeZq9.js` (537 bytes). |
...
| M-10 | ◐ partial 2026-05-05 | Frontend half (M-10a, F-H-02) shipped — `frontend/Dockerfile` base swapped from `nginx:alpine` to `nginxinc/nginx-unprivileged:alpine`; master + workers now run as uid 101 (nginx). Verified: `ps -ef` inside container reports `USER nginx PID 1`, image `--build`-recreated, healthcheck flipped to `healthy`, served bundle hash matches the just-built image, port 3000 binding works (no remap needed since 3000 is already unprivileged). Commit `578a4b8`. Backend half (M-10b, F-H-01) shipped as documentation: option (a) USER wairz directive at image level conflicts with the runtime `groupmod`/`usermod` GID fix-up in `entrypoint.sh:1-13` (both require root). Option (b) was lifting the existing entrypoint-level explanation up to a Dockerfile-level comment block at the `ENTRYPOINT` line, citing the trade-off and listing two future paths (privileged init container; build-time-only DOCKER_GID). Commit `e3d4f1b`. Genuine non-root backend would require either a privileged init container that chowns docker.sock to a fixed UID OR removal of the runtime GID fix-up — both are larger scope than a quick-win close-out and remain documented in the Dockerfile comment block. |
