...
- P3 residual promotion: `attack_surface_service.py`(2), `clamav_service.py`(2 — pure-leaf, hash_lookups import target), `hardware_firmware/cve_matcher.py`(2), 12 more files with 1 each — all gated on actual cycle pressure (Rule #19: evidence-first), not aesthetic. `firmware_service.py` cleared this session — no longer in the queue.
