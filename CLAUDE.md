...
23. **For parallel agent dispatch (Citadel Fleet / `Agent` tool with `isolation: "worktree"` or similar harnesses), the `worktreePath: "ok"` sentinel does NOT guarantee working-tree isolation.** In session 435cb5c2 three of six parallel streams showed cross-stream file-sweep via a shared on-disk checkout: Alpha's D3 commit (f614c43) swept two of Gamma's FindingsList / FindingsPage virtualization diffs; Delta's arq-worker commit (e8548fd) swept eight of Zeta's frontend API_BASE migration files; Zeta's commit-1 content ended up attributed to a Delta SHA. Content was preserved in all three cases but commit attribution, `git bisect` coherence, and per-stream rollback all broke. Mitigations, in order of preference: (a) the Fleet harness creates TRUE worktrees — a unique `git worktree add <path>` per stream; confirm via `git worktree list`; the `worktreePath: "ok"` sentinel is a marker, not a path; (b) each sub-agent runs `git checkout -b feat/stream-{name}-{YYYY-MM-DD}` BEFORE any file write, and the orchestrator merges branches sequentially post-completion; rollback becomes `git branch -D feat/stream-{name}-{YYYY-MM-DD}`; (c) design waves for AGGRESSIVE top-directory-level file-disjointness (backend/models/ vs backend/routers/ vs frontend/pages/ — truly separate subtrees) and accept the attribution noise. Strict `git add <specific-paths>` discipline alone is NOT sufficient — Wave-2 prompts used it uniformly and still failed 2 of 3 streams, because the shared on-disk checkout exposes unstaged changes from OTHER streams to the `git add` invocation. See `.planning/knowledge/wairz-intake-sweep-wave12-antipatterns.md` anti-pattern #1 for the full evidence matrix.
24. **For frontend typecheck in wairz, use `npx tsc -b --force` — never `tsc --noEmit`.** The `frontend/tsconfig.json` uses `"files": []` + project `"references"`; `--noEmit` has no files to check in the root project and exits 0 without descending into the referenced projects. This is a Rule 17 silent-CLI-exit instance — discovered via canary in session 435cb5c2 Wave 1 Stream Gamma after Gamma's first "0 errors" typecheck appeared to pass despite known type problems. The `-b --force` variant invokes all referenced projects and catches errors. Package.json `"typecheck"` scripts, any harness `typecheck.command` config, CI workflows, and agent-prompt verification steps must use `-b --force`. Mandatory Rule-17 canary once per session before trusting any "0 errors" output: `echo 'const x: number = "nope"; export default x;' > frontend/src/__canary.ts && (cd frontend && npx tsc -b --force); rm frontend/src/__canary.ts` — expect the command to fail with a type error before it's trusted for real use. The stale `"command": "npx tsc --noEmit"` entry in `.claude/harness.json:6` is a known companion defect; fix when the protect-files hook permits (see Phase 7 maintenance).
...
- `.mex/context/{architecture,stack,conventions,decisions,mcp-tools}.md` — dense, YAML-fronted context files with `triggers` + `edges` for graph navigation. The Verify Checklist in `conventions.md` is the task-time gate derived from rules 1–22 above.
