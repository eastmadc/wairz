...
- P3 residual promotion: 12 files each carry exactly 1 runtime function-body `app.*` import (`abusech_service`, `binary_analysis_service`, `comparison_service`, `device_service`, `hardware_firmware/classifier`, `hardware_firmware/graph`, `qiling_service`, `sbom/enrichment`, `sbom/strategies/syft_strategy`, `security_audit/network`, `virustotal_service`, `yara_service`). All 1-each distribution is the structural floor — further work is gated on actual cycle pressure (Rule #19: evidence-first), not aesthetic. Session aa8b4a17 (2026-04-24) recommended CLOSE of the P3 thread; the three dense residuals (`attack_surface_service`, `clamav_service`, `cve_matcher`) cleared this session.
...
**Known issues:**
- (Queue is otherwise clean. Next work item comes from operational pressure, not the backlog.)
