...
#### Phase 1: Foundation — Backend Scaffold (Sessions 1–5) DONE

Working backend with project CRUD, firmware upload, unpacking, and file browsing.

- **1.1** Project setup: FastAPI app, pydantic-settings config, async SQLAlchemy + Alembic, docker-compose (postgres + redis)
- **1.2** Project & firmware CRUD: Pydantic schemas, firmware_service for file storage + SHA256, routers for projects and firmware upload
- **1.3** Firmware unpacking: binwalk extraction, architecture/endianness detection via pyelftools, project status pipeline
- **1.4** File explorer API: list_directory, read_file, file_info, search_files with path traversal prevention via `os.path.realpath()` + prefix check
- **1.5** Backend Dockerfile (python:3.12-slim + system deps), docker-compose integration, .env.example

#### Phase 2: Frontend Foundation (Sessions 6–9) DONE

React frontend with project management, file explorer, and file viewer.

- **2.1** React + Vite + TypeScript scaffold, Tailwind + shadcn/ui, react-router-dom routing, axios API client
- **2.2** Project dashboard: Zustand projectStore, ProjectList/ProjectCard, FirmwareUpload with drag-and-drop + progress
- **2.3** File explorer: react-arborist FileTree (lazy-load), FileViewer (text/binary/image), BinaryInfo (ELF headers)
- **2.4** Monaco editor: syntax highlighting, HexViewer (offset|hex|ASCII with pagination), text/hex switching

#### Phase 3: AI Chat Integration (Sessions 10–14) DONE

AI chat panel with Claude tool-use loop, streaming, and filesystem/string tools.

- **3.1** Tool registry framework (ToolRegistry, ToolContext), filesystem tools (list_directory, read_file, file_info, search_files, find_files_by_type), output truncation (30KB max)
- **3.2** AI orchestrator: streaming tool-use loop with Anthropic API, max 25 iterations guard, system_prompt.py
- **3.3** WebSocket chat endpoint, conversation CRUD REST endpoints, message persistence in JSONB
- **3.4** Chat frontend: Zustand chatStore, useWebSocket hook, ChatPanel/ChatMessage/ToolCallBlock components with markdown rendering
- **3.5** String tools: extract_strings (categorized), search_strings (grep), find_crypto_material, find_hardcoded_credentials (entropy analysis)

#### Phase 4: Binary Analysis (Sessions 15–19) DONE

Binary analysis via radare2 and Ghidra headless decompilation.

- **4.1** Radare2 integration: r2pipe R2Session wrapper with LRU session caching, 60s analysis timeout
- **4.2** Binary AI tools: list_functions, disassemble_function, list_imports/exports, xrefs_to/from, get_binary_info, check_binary_protections (NX, RELRO, canaries, PIE, Fortify)
- **4.3** Ghidra headless decompilation: DecompileFunction.java script, subprocess runner, results cached in analysis_cache by binary_sha256 + function_name
- **4.4** Security tools: check_known_cves, analyze_config_security (shadow/passwd/sshd_config/httpd.conf), check_setuid_binaries, analyze_init_scripts, check_filesystem_permissions
- **4.5** Frontend binary viewer: protection status display, function list with search, disassembly/decompile views in Monaco, "Ask AI about this file" context action

#### Phase 5: Findings & Reporting (Sessions 20–22) DONE

Structured findings management and report export.

- **5.1** Findings CRUD backend + frontend: FindingsList (sortable/filterable table, severity badges), FindingDetail (evidence, file navigation, status management)
- **5.2** Report export: POST endpoint, Markdown + PDF formats, executive summary + findings by severity + appendix

#### Phase 6: Firmware Component Map (Sessions 23–25) DONE

Interactive graph visualization of firmware component dependencies.

- **6.1** Component graph backend: dependency analysis engine parsing ELF DT_NEEDED, imported functions per library, shell script calls, init script→service mapping, config file→binary references. Graph data model (ComponentNode/Edge/Graph), cached in analysis_cache, capped at 500 nodes
- **6.2** Frontend: ReactFlow (@xyflow/react) canvas with dagre layout, typed/colored nodes (binary=blue, library=purple, script=green, config=orange, init=yellow), styled edges by relationship type, EdgeDetails popover, MapControls (filter/search/layout toggle)
- **6.3** AI `get_component_map` tool, right-click context menu AI integration, cluster collapsing for large graphs (200+ nodes), PNG/SVG/JSON export

#### Phase 7: AI Decompiled Code Clean-up (Sessions 26–27) DONE

AI-powered cleanup of Ghidra decompiled code.

- **7.1** Backend code_cleanup_service.py: Claude API call with specialized prompt for variable/function renaming, inline comments, security annotations. Cached as `code_cleanup:{function_name}` in analysis_cache. POST `/analysis/cleanup` endpoint. AI tool `cleanup_decompiled_code`
- **7.2** Frontend: "Clean Up with AI" button, toggle between raw/cleaned views in Monaco, "AI Cleaned" badge, loading/error states

#### Phase 9: SBOM & Known Vulnerability Look-Up (Sessions 32–34) DONE

SBOM generation from firmware filesystem + CVE lookup via NVD.

- **9.1** SBOM generation: SbomComponent/SbomVulnerability SQLAlchemy models, sbom_service.py with 5 detection strategies (package manager DBs, kernel version, library SONAME, binary version strings, config file hints). CPE 2.3 + PURL identifiers. CycloneDX JSON export via cyclonedx-python-lib
- **9.2** Vulnerability lookup: vulnerability_service.py querying NVD via nvdlib with rate limiting (asyncio.Semaphore). Auto-creates findings with `source='sbom_scan'` grouped by component. REST endpoints for SBOM CRUD, export, vulnerability scan/results/summary. AI tools: generate_sbom, get_sbom_components, check_component_cves, run_vulnerability_scan
- **9.3** Frontend SbomPage: SBOM tab (component table, filter/search, export), Vulnerabilities tab (severity dashboard cards, CVE table, finding links). FindingsList updated with source column/badge and "Inherited Vulnerability" treatment for SBOM findings
